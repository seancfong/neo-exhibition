...
const ExperienceCard = ({ experienceData }: ExperienceCardType) => {
  ...
  return (
    <>
      {/* Mobile Card */}
      <div className="relative z-10 grid h-fit max-h-[80vh] min-w-[330px] max-w-[min(90vw,600px)] flex-1 grid-cols-5 grid-rows-[auto_auto_1fr] lg:max-w-3xl lg:overflow-y-auto">
        ...
        <div className="col-start-1 row-start-1 flex h-fit w-fit justify-end gap-2 justify-self-end overflow-x-visible pb-3 pr-2 pt-10">
          {/* Date Container */}
          <div className="relative h-fit whitespace-nowrap pt-5 text-right text-sm text-dim sm:text-base lg:text-lg">
            <p>{startDate}</p>
            <p className="pr-1">|</p>
            <p>{endDate}</p>
          </div>
        ...
        {/* Content Card (r1, c2) */}
        <div className="relative col-start-2 col-end-6 row-start-1 row-end-3 flex flex-col gap-3 p-5 lg:gap-4 lg:px-10 lg:py-6">
          {/* Logo */}
          ...
          <h4 className="text-3xl font-light leading-8 tracking-wide sm:text-4xl lg:text-5xl">
            {position}
          </h4>
        ...
        <div className="col-start-2 col-end-6 row-start-3 rounded-br-lg bg-panel-darkest p-5 pt-3">
          <ul className="list-inside font-light tracking-wider sm:text-lg lg:text-2xl">
            {bullets.map((bullet) => {
          ...
          <ul className="flex flex-wrap gap-x-3 gap-y-2 text-xs font-light uppercase tracking-widest text-dim sm:text-sm lg:text-base">
            {skills?.map((skill, i) => {
